function [submerged] = piSceneSubmergeRectangles(thisR, medium, varargin)
p = inputParser;
...
dx = 0.02*inputs.sizeX/2;
dy = inputs.sizeY/2;
dz = 0.1*inputs.sizeZ/2;

for i = 1:nearestDiv(inputs.sizeX,dx)
    ...
    water.size.pmin = [-dx; -dy; -dz];
    water.size.pmax = [dx; dy; dz];
    water.translation = {[-inputs.sizeX/2 + dx/2 + k; inputs.offsetY; inputs.sizeZ/2 + 0.43]};
